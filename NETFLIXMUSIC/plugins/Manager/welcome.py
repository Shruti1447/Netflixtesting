...
def welcomepic(pic_path, user, chatname, user_id, uname):
    background = Image.open("NETFLIXMUSIC/assets/shrushthi/NetflixNwel.png")
    pfp = Image.open(pic_path).convert("RGBA")
    pfp = circle(pfp, size=(835, 839))
    draw = ImageDraw.Draw(background)
    font_large = ImageFont.truetype('NETFLIXMUSIC/assets/shrushthi/ArialReg.ttf', size=65)
...
async def auto_state(client, message):
    ...
    if user_status.status not in (enums.ChatMemberStatus.ADMINISTRATOR, enums.ChatMemberStatus.OWNER):
        return await message.reply_text("**Sorry, only admins can change the welcome notification status.!**")
...
async def greet_new_member(client, member: ChatMemberUpdated):
    ...
    if not welcome_enabled:
        ...
        if auto_reenabled:
            await client.send_message(
                chat_id,
                "**Welcome messages have been automatically re-enabled.**"
            )
        else:
            return
    ...
    if join_count >= 10:
        ...
        await client.send_message(
            chat_id,
            "**Massive join detected.Welcome messages are temporarily disabled for 30 minutes.**"
        )
        return
    ...
    if member.new_chat_member and member.new_chat_member.status == enums.ChatMemberStatus.MEMBER:
        try:
            ...
            if user.photo:
                ...
                pic_path = "NETFLIXMUSIC/assets/upic.png"
...
""",
            ...
            if pic_path and os.path.exists(pic_path) and "NETFLIXMUSIC/assets/upic.png" not in pic_path:
                os.remove(pic_path)
