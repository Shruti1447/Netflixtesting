...
@app.on_message(filters.command(["shrushthis"], prefixes=["j", "J"]))
async def gpt_handler(client: Client, message: Message):
    try:
        ...
        if len(message.command) < 2:
            await message.reply_text(f"Hello {name}, I am Shrushthi. How can I help you today?")
            return
...
async def chat_gpt(client: Client, message: Message):
    try:
        ...
        if len(message.command) < 2:
            await message.reply_text(f"🚀 Hey {name}, ready to explore something amazing? 🎧")
            return
...
async def chat_shrushthi(client: Client, message: Message):
    try:
        await client.send_chat_action(message.chat.id, ChatAction.RECORD_AUDIO)
        name = message.from_user.first_name

        if len(message.command) < 2:
            await message.reply_text(f"Hello {name}, I am Netflix. How can I assist you today?")
            return
