...
from NETFLIXMUSIC.utils import close_markup
from config import BANNED_USERS, SERVER_PLAYLIST_LIMIT
from NETFLIXMUSIC import Carbon, app
from NETFLIXMUSIC.utils.decorators.language import language, languageCB
from NETFLIXMUSIC.utils.inline.playlist import (botplaylist_markup,
                                              get_playlist_markup,
                                              warning_markup)
...
async def add_playlist(client, message: Message, _):
    ...
    "**🎵 Please provide a song name, song link, or YouTube playlist link after the command.**\n\n"
    "**📌 Examples:**\n\n"
    "🔹 `/addplaylist Blue Eyes` — *(Add a specific song by name)*\n"
    "🔹 `/addplaylist [YouTube Playlist Link]` — *(Add all songs from a YouTube playlist to the bot playlist.)*"
)
    ...
    if "youtube.com/playlist" in query:
        adding = await message.reply_text("**🎧 Adding Songs in Playlist Please wait..**")
        try:
        ...
        if not video_urls:
            return await message.reply_text("**➻ No Songs Found in the Playlist Links.\n\n**➥ Try Other Playlist Link**")
        ...
        for video_url in video_urls:
            ...
            [
                [
                    InlineKeyboardButton("๏ Want Remove Any Songs? ๏", callback_data=f"open_playlist {user_id}")
                ]
            ]
        )
        await adding.delete()
        return await message.reply_text(text="**➻ All Songs Has been Added Successfully From Your Youtube Playlist links✅**\n\n**➥ if You Want to Remove any song then clik Given Below Button.\n\n**▷ Chcek By » /playlist**\n\n▷ **Play By » /play**", reply_markup=keyboardes)
        pass
    if "youtube.com/@" in query:
        addin = await message.reply_text("**🎧 Addding songs in Playlist Please wait..**")
        try:
            from pytube import YouTube
        ...
        if not video_urls:
            return await message.reply_text("**➻ No Songs Found in the YouTube Channel.\n\n**➥ Try Other Youtube Channel Links**")
        ...
        await addin.delete()
        return await message.reply_text(text="**➻ All Songs Has Been Added Sucessfull From Your Youtube Channel links✅**\n\n**➥ if You want to Remove Any Song then click given below.\n\n**▷ Chcek By » /playlist**\n\n▷ **Play » /play**", reply_markup=keyboardes)
        pass
    # Check if the provided input is a YouTube video link
    if "https://youtu.be" in query:
        try:
            add = await message.reply_text("**🎧 Adding Song in playlist please wait..**")
            from pytube import Playlist
            from pytube import YouTube
            ...
            try:
                ...
                await add.delete()
                await message.reply_photo(thumbnail, caption="**➻ Added song in your bot playlist✅**\n\n**➥ check  » /playlist**\n\n**➥ Delete By » /delplaylist**\n\n**➥ And Play By » /play (Group Only)**", reply_markup=keyboard)
            except Exception as e:
                print(f"Error: {e}")
        ...
        from NETFLIXMUSIC import YouTube
        ...
        try:
            ...
            m = await message.reply("**🔄 Adding Please wait... **")
            ...
            await m.delete()
            await message.reply_photo(thumbnail, caption="**➻ Added song in your bot Playslist✅**\n\n**➥ Check by » /playlist**\n\n**➥ Delete by » /delplaylist**\n\n**➥ and play By » /play (Groups only)**", reply_markup=keyboard)
...
async def del_plist(client, CallbackQuery, _):
    ...
    keyboards = InlineKeyboardMarkup(
            [
                [
                    InlineKeyboardButton("๏ Recover Your Song ๏", callback_data=f"recover_playlist {videoid}")
                ]
            ]
        )
    return await CallbackQuery.edit_message_text(
    text="➻ Your song has been deleted from your bot playlist.\n\n➥ If you want to recover your song in your playlist, then click the button below."
...
async def add_playlist(client, CallbackQuery, _):
    ...
    try:
        ...
        keyboardss = InlineKeyboardMarkup(
            [
                [
                    InlineKeyboardButton("๏ Remove Again ๏", callback_data=f"remove_playlist {videoid}")
                ]
            ]
        )
        return await CallbackQuery.edit_message_text(text=
        "✅ **Recovered Song in Your Playlist!**\n\n"
        "🎵 **Manage Your Playlist:**\n"
        "➜ **Check Playlist:** `/playlist`\n"
        "➜ **Delete Playlist:** `/delplaylist`\n"
        "➜ **Play Playlist:** `/play",
...
async def add_playlist(client, CallbackQuery, _):
    await CallbackQuery.answer(
    "🎵 **To add a song to your playlist, type:**\n"
...
@app.on_callback_query(filters.regex("shrushthi_playlist") & ~BANNED_USERS)
@languageCB
async def add_playlist(client, CallbackQuery, _):
    callback_data = CallbackQuery.data.strip()
    videoid = callback_data.split(None, 1)[1]
...
async def del_whole_playlist(client, CallbackQuery, _):
    ...
    _playlist = await get_playlist_names(CallbackQuery.from_user.id)
    for x in _playlist:
        await CallbackQuery.answer("➻ ok sir please wait.\n\n➥ Deleting Your Playlist...", show_alert=True)
