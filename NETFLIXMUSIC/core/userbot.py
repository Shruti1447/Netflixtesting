...
class Userbot:
    ...
    async def start(self):
        LOGGER(__name__).info("Netflix's Assistant starting...")
        if self.one:
            ...
            assistants.append(1)
            try:
                await self.one.send_message(config.LOGGER_ID, "Netflix's Assistant 1 Started")
            except Exception as e:
        ...
        if self.two:
            ...
            try:
                await self.two.join_chat("Beats_Support")
            except Exception as e:
                LOGGER(__name__).warning(f"Assistant 2 failed to join chats: {e}")
            assistants.append(2)
            try:
                await self.two.send_message(config.LOGGER_ID, "Netflix's Assistant 2 Started")
            except Exception as e:
                LOGGER(__name__).error(
        ...
        if self.three:
            ...
            try:
                await self.three.join_chat("Beats_Support")
            except Exception as e:
                LOGGER(__name__).warning(f"Assistant 3 failed to join chats: {e}")
            assistants.append(3)
            try:
                await self.three.send_message(config.LOGGER_ID, "Netflix's Assistant 3 Started")
            except Exception as e:
                LOGGER(__name__).error(
        ...
        if self.four:
            ...
            try:
                await self.four.join_chat("Beats_Support")
            except Exception as e:
                LOGGER(__name__).warning(f"Assistant 4 failed to join chats: {e}")
            assistants.append(4)
            try:
                await self.four.send_message(config.LOGGER_ID, "Netflix's Assistant 4 Started")
            except Exception as e:
                LOGGER(__name__).error(
        ...
        if self.five:
            ...
            try:
                await self.five.join_chat("Beats_Support")
            except Exception as e:
                LOGGER(__name__).warning(f"Assistant 5 failed to join chats: {e}")
            assistants.append(5)
            try:
                await self.five.send_message(config.LOGGER_ID, "Netflix's Assistant 5 Started")
            except Exception as e:
                LOGGER(__name__).error(
    ...
    async def stop(self):
        LOGGER(__name__).info("Netflix's assistants stopping...")
        if self.one:
            try:
                await self.one.stop()
            except Exception as e:
